...
#
#  $RCSfile: Bilateral_Ankle_Exo.mk $ $Revision: 1.34 $ $Date: 2017/06/24 12:16:10CEST $
...
MAKEFILE                       = Bilateral_Ankle_Exo.mk
...
MATLAB_ROOT                    = S:\MATLAB\R2017b
MATLAB_BIN                     = S:\MATLAB\R2017b\bin
MATLAB_ROOT_ALT                = S:\MATLAB\R2017b
MATLAB_BIN_ALT                 = S:\MATLAB\R2017b\bin
RELEASE_VERSION                = R2017b
RELEASE_VERSION               := $(RELEASE_VERSION,S'\.'_',W_,LC,S'^r'')
COMPUTER                       = PCWIN64
MODEL                          = Bilateral_Ankle_Exo
MODULES                        = Bilateral_Ankle_Exo_data.c Bilateral_Ankle_Exo_trc_ptr.c rtGetInf.c rtGetNaN.c rt_nonfinite.c
S_FUNCTIONS                    = 
S_FUNCTIONS_LIB                = 
SOLVER                         = 
NUMST                          = 3
...
%setenv QNX_HOST              = $(DSPACE_CONFIG)\$(CFG_RTI_QNX_HOST)         # export QNX_HOST to DOS environment. It is needed by ntoppc-*.exe
%setenv QNX_TARGET            = $(DSPACE_CONFIG)\$(CFG_RTI_QNX_TARGET)       # export QNX_TARGET to DOS environment. It is needed by ntoppc-*.exe
%setenv PATH                  = $(PATH);$(DSPACE_CONFIG)\$(CFG_RTI_QNX_PATH) # (this is important because we need the cygwin1.dll)
%setenv ZIP                   =
%setenv ZIPOPT                =
%setenv ZIP_OPTS              =

COMP_ROOT = "$(DSPACE_CONFIG)\$(CFG_RTI_QNX_BASE)"
COMP_VS_DISP = (GCC Vs $(CFG_RTI_QNX_GCC_VERSION))
COMP_BIN = "$(DSPACE_CONFIG)\$(CFG_RTI_QNX_PATH)"

%foreach ITEM in $(CFG_RTI_QNX_HDR_DIRPATHS)
%  set COMP_INC += "$(DSPACE_CONFIG)\$(ITEM)"
%endfor
%set COMP_INC += "$(QNX_TARGET)\usr\include"
...
# RTI task mode options
%ifdef TASK_MODE
% if member($(TASK_MODE,UC), LOW_JITTER STANDARD)
%   set TASK_MODE_DEFINE = -DTASK_MODE=$(TASK_MODE,UC)
% else
%   set TASK_MODE_DEFINE =
% endif
%endif
...
RTWCUST_INCLUDES = \
    ...
    "$(START_DIR)\Bilateral_Ankle_Exo_rti1202"
...
start_check :
    ...
    %endif
    %ifdef TASK_MODE
      %if !member($(TASK_MODE), LOW_JITTER STANDARD)
        %echo ERROR: Invalid TASK_MODE option '$(TASK_MODE)' specified.
        %echo        Select LOW_JITTER or STANDARD.
        %do make_abort
      %endif
    %endif
    %ifdef SSCALE
